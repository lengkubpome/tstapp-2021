...
import { ContactState, ContactStateModel } from 'src/app/shared/state/contact/contact.state';
import { IContact } from 'src/app/shared/models/contact.model';
import { ProductState, ProductStateModel } from 'src/app/shared/state/product/product.state';
...
export class WeightSheetComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.products = this.store.selectSnapshot<ProductStateModel>(ProductState).products;

    this.contacts = this.store.selectSnapshot<ContactStateModel>(ContactState).contacts;
    ...
    this.weightingForm = this.formBuilder.group({
      ...
      contact: ['', Validators.compose([inList(this.contacts, ['displayName'])])],
      product: ['', Validators.compose([Validators.required, inList(this.products, ['name'])])],
      price: ['', Validators.compose([Validators.pattern('(^[0-9]*[.]?[0-9]{0,2})')])],
      cutWeight: ['', Validators.compose([Validators.pattern('(^[0-9]*[.]?[0-9]*[%]?)')])],
      notes: [],
      liveWeight: [23044],
    });
  ...
  private filterControls(): void {
    this.filteredCars = this.weightingForm.get('car').valueChanges.pipe(
      ...
      startWith(''),
      debounceTime(300),
      map((inputValue) => (inputValue ? this._filterCar(inputValue) : this.cars.slice()))
    );
    ...
    this.filteredContacts = this.weightingForm.get('contact').valueChanges.pipe(
      ...
      startWith(''),
      debounceTime(300),
      map((inputValue) => (inputValue ? this._filterContact(inputValue) : this.contacts.slice()))
    );
    ...
    this.filteredProducts = this.weightingForm.get('product').valueChanges.pipe(
      ...
      startWith(''),
      debounceTime(300),
      map((inputValue: string) => (inputValue ? this._filterProducts(inputValue) : this.products.slice()))
    );

    this.filteredWeightingTypes = this.weightingForm.get('type').valueChanges.pipe(
      takeUntil(this.unsubscribeAll),
      startWith(''),
      debounceTime(300),
      map((inputValue) => (inputValue ? this._filterWeightingType(inputValue) : this.weightingTypes.slice()))
    );
  }

  private _filterWeightingType(value: string): IWeightingType[] {
    const filterValue = this._normalizeValue(value);
    return this.weightingTypes.filter((type) => this._normalizeValue(type.th).includes(filterValue));
  }

  private _filterCar(value: string): ICar[] {
    const filterValue = this._normalizeValue(value);
    return this.cars.filter((car) => this._normalizeValue(car.plateLCN + car.plateLCP).includes(filterValue));
  }

  private _filterContact(value: string): IContact[] {
    const filterValue = this._normalizeValue(value);
    return this.contacts.filter((contact) => this._normalizeValue(contact.displayName).includes(filterValue));
  }

  private _filterProducts(value: string): IProduct[] {
    const filterValue = this._normalizeValue(value);
    return this.products.filter((product) => this._normalizeValue(product.id + product.name).includes(filterValue));
  }
