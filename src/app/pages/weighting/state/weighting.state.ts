...
export class WeightingState implements NgxsOnInit {
	...
	ngxsOnInit(ctx: StateContext<WeightingStateModel>): void {
		console.log(
			"State initialized, now getting weight sheet and weighting type"
		);
	...
	// @Action(WeightingAction.GenerateID)
	// generateID(ctx: StateContext<WeightingStateModel>): Void {return null}

	@Action(WeightingAction.CreateWeightSheet)
	add(
		ctx: StateContext<WeightingStateModel>,
		{ payload }: WeightingAction.CreateWeightSheet
	): void {
		const state = ctx.getState();
		ctx.setState({
			...state,
